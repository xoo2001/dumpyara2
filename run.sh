...
apt-get -y update && apt-get -y upgrade && apt-get install -y unace unrar zip unzip p7zip-full p7zip-rar sharutils rar uudeview mpack arj cabextract device-tree-compiler liblzma-dev python3 python3-pip brotli liblz4-tool axel gawk aria2 detox cpio rename wget liblz4-dev curl python3-venv
python3 -m venv .venv
